		...
		for obj in blender_armatures:
			...
			for i in range(frame_count):
				...
				for bone_data in ordered_bones:
					...
					#quat = grassman(quat, pose_bone.quat)#translate armature to bone pose
					...
					if blender_bone.hasParent():
						...
						parent_head = child_parent.head['BONESPACE']* quat_parent
						parent_tail = child_parent.tail['BONESPACE']* quat_parent
						a_head = (parent_tail - parent_head) + blender_bone.head['BONESPACE']
						
						p_parent = pose_bone.parent
						p_parent_quat = p_parent.quat.inverse()
						p_parent_head = p_parent.head * p_parent_quat
						p_parent_tail = p_parent.tail * p_parent_quat
						
						head = ((p_parent_head - p_parent_head) + pose_bone.head) + a_head
