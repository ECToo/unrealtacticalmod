...
#==================================================================================================    
  ...
  while counter < recCount:
    ...
    #indata = unpack('64sLii',pskfile.read(120))#print indata[2] #NumChld#print "Name:",rstrip(indata[0][:30]),"NoChild",indata[2],"INDEX",indata[3],"ID",counter-1
    print "Name:",strip(indata[0][:6]),"|||"
    #print "Name:",rstrip(indata[0][:30]),"Parent ID",indata[3],"ID",counter-1#print "Name:",rstrip(indata[0][:30]),"x:",indata[8],"y:",indata[9],"z:",indata[10] #,"Parent ID",indata[3],"ID",counter-1#print "Name:",rstrip(indata[0][:30]),"Flgs:",indata[1],"PrntIdx:",indata[3]#print "Name:",rstrip(indata[0][:30]),"Leng:",indata[11],"XSize:",indata[12],"YSize:",indata[13],"ZSize:",indata[14]#print len(rstrip(indata[0][:30]))
    bone.append(indata)
    bnstr = rstrip(indata[0])
    Bns.append(bnstr)
...
#==================================================================================================        
  ...
  armObj = Object.New('Armature', "Armature") #Object Create for Armature
  armData = Armature.New('Armature') #Main Armature Data
  arms = Armature.Get('Armature')
  print arms, "Armature DATA"
  armData.drawType = Armature.OCTAHEDRON
  armData.makeEditable()
  for armbone in bone:
    print "-----"
    IDNo = armbone[3]
    IDname = rstrip(bone[IDNo][0][:30])
    name = rstrip(armbone[0][:30])
  ...
  for arm in arms_current.values():
    ...
    if arm.name == 'Armature':
       ...
       eb = Armature.Editbone()
       eb.roll = 10
       eb.parent = arm.bones['joint1']
       eb.head = Vector(1,1,1)
       eb.tail = Vector(0,1,1)
       ...
       eb = Armature.Editbone()
       eb.roll = 10
       eb.parent = arm.bones['myNewBone']
       parentbone = arm.bones['myNewBone']
       eb.head = Vector(1,1,1) + parentbone.head
       eb.tail = Vector(0,1,1) + parentbone.head
       arm.bones['myNewBone2'] = eb
       arm.update()  #save changes
       ...
       eb = Armature.Editbone()
       eb.roll = 10
       eb.parent = arm.bones['myNewBone2']
       parentbone = arm.bones['myNewBone2']
       eb.head = Vector(1,1,1) + parentbone.head
       eb.tail = Vector(0,1,1) + parentbone.head
  ...
  for armbone in bone:
    ...
    if name != IDname:
       ...
       arms_current.makeEditable()
       arms_current.bones[name] = eb #Add Bone to Armature.
       arms_current.update()  #save changes
  """
  """
  arms = Armature.Get()
  ...
  """
  armObj = Object.New('Armature', "Armature") #Object Create for Armature
  armData = Armature.New('Armature_Unreal') #Main Armature Data
  arms = Armature.Get('Armature')
  print arms, "Armature DATA"
  armData.drawType = Armature.OCTAHEDRON
  armData.makeEditable()
  for armbone in bone:
    #print armbone[0][:30]
    print "-----"
    #print "Name:",rstrip(armbone[0][:30])
    IDNo = armbone[3]
    IDname = rstrip(bone[IDNo][0][:30])
    name = rstrip(armbone[0][:30])
    print "Name:",name,"Parent Name:",IDname
    eb = Armature.Editbone()
    if name != IDname: #parent add
       #print "THIS BONE:<"
       #print "Parent Bone:",IDname
       #eb.parent = arms.bones[IDname]
       eb.parent = arm.bones[rstrip(arm.bones[IDname].name)]
       print arm.bones[IDname].name
    eb.head = Vector(armbone[8],armbone[9],armbone[10])
    eb.tail = Vector(armbone[8],armbone[9],armbone[10] + 1)
    #print "Bone Position: x:",armbone[8]," y:", armbone[9]," z:", armbone[10]
    arm.makeEditable()
    arm.bones[name] = eb #add bones
    arm.update()
  armObj.link(arm)
  armData.update()
  scn = Blender.Scene.getCurrent()
