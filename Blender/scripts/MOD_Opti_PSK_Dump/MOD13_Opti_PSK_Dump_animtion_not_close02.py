#!BPY
""" 
Name: 'Unreal Skeletal Mesh/Animation (.psk and .psa) Mod13' 
Blender: 245
Group: 'Export' 
...
def parse_animation(blender_scene, psa_file):
	...
	for act in Armature.NLA.GetActions().values():
		...
		for obj in blender_armatures:
			...
			act.setActive(obj)
			#====================================
			pose = obj.getPose() #pose bone
			armature_data= obj.getData()
			p_bones = pose.bones.values()
			...
			for i in range(frame_count):
				...
				for bone_data in ordered_bones:
					...
					matrix = Mathutils.Matrix(pose_bone.poseMatrix)
					#print "MATRIX", matrix
					#rest_matrix = Mathutils.Matrix(blender_bone.matrix['ARMATURESPACE'])
					rest_matrix = blender_bone.matrix['ARMATURESPACE']
					parent_bone = blender_bone.parent
					if parent_bone:
					...
					head = Mathutils.Vector(matrix.translationPart()) + (pose_bone.head - blender_bone.head['ARMATURESPACE'])
					quat = Mathutils.Quaternion(matrix.toQuat())
