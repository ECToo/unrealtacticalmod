#!BPY
""" 
Name: 'Unreal Skeletal Mesh/Animation (.psk and .psa) Mod07' 
Blender: 240 
Group: 'Export' 
...
def parse_bone(blender_bone, psk_file, psa_file, parent_id, is_root_bone, parent_mat):
	...
	if (parent_mat):
		head = blender_bone.head['ARMATURESPACE'] * parent_mat #ARMATURESPACE
		tail = blender_bone.tail['ARMATURESPACE'] * parent_mat #ARMATURESPACE
		rot_mat = blender_bone.matrix['BONESPACE'] * parent_mat.rotationPart() #ARMATURESPACE
		quat = make_fquat(rot_mat.toQuat())
	else:
		head = blender_bone.head['ARMATURESPACE'] #ARMATURESPACE
		tail = blender_bone.tail['ARMATURESPACE'] #ARMATURESPACE
		quat = make_fquat(blender_bone.matrix['BONESPACE'].toQuat()) #ARMATURESPACE
		
	#print blender_bone.matrix
	...
	bone_mat = blender_bone.matrix['ARMATURESPACE'] #ARMATURESPACE #BONESPACE'
	...
	for current_child_bone in blender_bone.children:
		global my_id
		#print "my_id:",current_id
		parse_bone(current_child_bone, psk_file, psa_file, my_id, 0, bone_mat)
...
def parse_armature(blender_armature, psk_file, psa_file):
	...
	for current_obj in blender_armature: 
		...
		child_count += len(bones)
	print "current_obj.mat:",current_obj.mat
	print "parse_armature child_count:",child_count
