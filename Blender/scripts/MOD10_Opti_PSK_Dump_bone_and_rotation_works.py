...
nbone = 0
def parse_bone(blender_bone, psk_file, psa_file, parent_id, is_root_bone, parent_mat,parent_root,blender_armature):
	global nbone 	# look it's evil!
	#This is the armature object to keep in here for test and bone look up.
	for current_obj in blender_armature: 
		current_armature = current_obj.getData() #Armature bone
		arm_mat = current_obj.matrixWorld #parent armature not bone
		
	arm = current_armature
	...
	if child_parent != None:
		...
		# ROOT BONE
		#This is the parent if parent is does not exist for it
	...
	if blender_bone.hasChildren():
		for current_child_bone in blender_bone.children:
			parse_bone(current_child_bone, psk_file, psa_file, my_id, 0, mainparent,parent_root,blender_armature)
...
def parse_armature(blender_armature, psk_file, psa_file):
	...
	for current_obj in blender_armature: 
		...
		for current_bone in bones:
			parse_bone(current_bone, psk_file, psa_file, 0, 0, current_obj.mat,None,blender_armature)
