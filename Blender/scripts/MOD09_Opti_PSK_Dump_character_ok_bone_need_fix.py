...
def parse_bone(blender_bone, psk_file, psa_file, parent_id, is_root_bone, parent_mat):
	...
	if  nbone == 0:#object root of armature
		...
		#head = blender_bone.head['BONESPACE'] + ((parent_mat.tail['BONESPACE']-parent_mat.head['BONESPACE']))
		#quat = make_fquat(blender_bone.matrix['BONESPACE'].toQuat())
		#quat = make_fquat(blender_bone.getRestMatrix['BONESPACE'].toQuat())
	else:
		#print parent_mat.length
		print "Other bone"
		#head = blender_bone.head['BONESPACE']
		head = (blender_bone.head['BONESPACE'] + (child_parent.tail['BONESPACE']-child_parent.head['BONESPACE']))
		print "LEN:",(child_parent.tail['BONESPACE']-child_parent.head['BONESPACE'])
		#head = blender_bone.head['BONESPACE'] + ((parent_mat.tail['BONESPACE']-parent_mat.head['BONESPACE']))
		...
		for obj in blender_armatures:
			...
			for i in range(frame_count):
				...
				for bone_data in ordered_bones:
					...
					#WOW
					head = (pose_bone.quat*(head))+(tail-head)+ pose_bone.loc
