...
def pskimport(infile):
  ...
  armObj = Object.New('Armature', "Armature_obj") #Object Create for Armature
  armData = Armature.New('Armature_Unreal') #Main Armature Data
  print "---PRASE--BONES---"
  while counter < recCount:
    counter = counter + 1
    indata = unpack('64s3i11f',pskfile.read(120))
    #indata = unpack('64sLii',pskfile.read(120))
    #print indata[2] #NumChld
    #print "Name:",rstrip(indata[0][:30]),"Parent ID",indata[3],"ID",counter-1
    print "Name:",rstrip(indata[0][:30]),"x:",indata[8],"y:",indata[9],"z:",indata[10] #,"Parent ID",indata[3],"ID",counter-1
    #print "Name:",rstrip(indata[0][:30]),"Flgs:",indata[1],"PrntIdx:",indata[3]
    #print "Name:",rstrip(indata[0][:30]),"Leng:",indata[11],"XSize:",indata[12],"YSize:",indata[13],"ZSize:",indata[14]
    #print len(rstrip(indata[0][:30]))
    bone.append(indata)
    bnstr = rstrip(indata[0])
    Bns.append(bnstr)
    #print str(indata[1][:30])
  ...
  for armbone in bone:
    ...
    if name != IDname: #parent add
       #print "THIS BONE:<"
       eb.parent = armData.bones[IDname]
    #print "Name of Parent:",IDname
    #print"POS:","x:",armbone[8],"y:",armbone[9],"z:",armbone[10]
    #eb.head = Vector(armbone[8],armbone[9],armbone[10])
    eb.head.x =armbone[8]
    eb.head.y =armbone[9]
    eb.head.x =armbone[10]
